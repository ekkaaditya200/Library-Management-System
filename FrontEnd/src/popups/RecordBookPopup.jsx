...
const RecordBookPopup = ({ bookId }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 p-5 flex items-center justify-center z-50">
      <div className="w-full bg-white rounded-lg shadow-lg md:w-2/3">
        <div className="p-6">
          ...
          <form onSubmit={handleRecordBook}>
            <div className="mb-4">
              ...
              <input type="email" name={email} onChange={(e)=>setEmail(e.target.value)} placeholder="Borrower's Email" className="w-full px-4 py-2 border-2 border-black rounded-md" />
            </div>
            ...
            <button type="submit" className="px-4 py-2 bg-black text-white rounded-md hover:bg-gray-800">Record Book</button>
            </div>
          </form>
        </div>
      </div>
